  ...
  try {
    ...
    for (const p of perfiles ?? []) {
      ...
      if (profile?.email) {
        await sendEmail(
          profile.email,
          `Nueva solicitud en tu zona: ${categoria}`,
          `
            <div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 2rem;">
              ...
              <div style="background: #fef3c7; border-radius: 12px; padding: 1.5rem; margin: 1.5rem 0;">
                <p style="color: #92400e; font-size: 14px; font-weight: 600; margin: 0;">📋 Nueva solicitud en ${comuna}</p>
              </div>
              <p style="color: #444; font-size: 14px; line-height: 1.6;">Hola ${profile.full_name ?? 'Proveedor'}, un cliente necesita <strong>${categoria}</strong>.</p>
              <p style="color: #444; font-size: 14px;">Fechas: ${solicitud.fecha_inicio} → ${solicitud.fecha_fin}</p>
              <div style="text-align: center; margin: 2rem 0;">
                <a href="${SITE_URL}/proveedor" style="background: #1dbf73; color: #fff; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: 600; font-size: 15px;">Ver solicitud</a>
              </div>
    ...
    return NextResponse.json({ ok: true, enviados, siteUrl: SITE_URL })
  } catch (error) {
